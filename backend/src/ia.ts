...
export type ResultRow = Record<string, {
  type: string;
  value: string;
}>;

export async function select({ clause, filters, orderBy, selection = ['*'] }: {
  clause, filters?, orderBy?, selection?
}): Promise<ResultRow[]> {
  let selectQuery = sparql.select(selection)
    .distinct()
    .where(clause);
