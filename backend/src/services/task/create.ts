...
export async function create(req: Request, res: Response) {
  ...
  const task = CreateTask.check(req.files?.length ? { ...JSON.parse(req.body.task), incidentId } : { ...req.body, incidentId });
  ...
  const { triples: attachmentTriples } = await extractAttachments(
    req,
    task.attachments,
    taskId,
    taskNode
  ...
  // Extract location triples if present
  ...
  const triples = [
    ...
    // Add attachment and location triples
    ...attachmentTriples,
    ...locationTriples
  ];
